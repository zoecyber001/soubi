      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b-2 border-cyan-neon/30 pb-4 gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold tracking-tighter text-white flex items-center gap-4">
            <span className={`text-cyan-neon ${deviceStatus.connected ? 'animate-pulse' : ''}`}>⬤</span>
            COMMAND CENTER
          </h1>
          <div className="flex items-center gap-4 mt-2 ml-10">
            <span className={`text-[10px] font-normal px-2 py-0.5 rounded border tracking-widest uppercase transition-all ${deviceStatus.connected
              ? 'text-cyan-neon bg-cyan-neon/10 border-cyan-neon/30'
              : 'text-dim bg-dim/10 border-dim/30'
              }`}>
              {deviceStatus.connected ? 'GHOST NODE CONNECTED' : 'SYSTEM READY'}
            </span>
            {deviceStatus.connected && (
              <span className="text-[10px] font-mono text-cyan-neon">
                BATTERY: {deviceStatus.battery || '100'}%
              </span>
            )}
            {!deviceStatus.connected && (
              <button
                onClick={() => useStore.getState().setDeviceStatus({ connected: true, battery: 100 })}
                className="ml-4 text-[10px] bg-cyan-neon/20 hover:bg-cyan-neon/40 text-cyan-neon px-2 py-1 rounded border border-cyan-neon/30 transition-all font-mono"
              >
                [SIMULATE CONNECTION]
              </button>
            )}
          ...
          {recentLogs.length === 0 ? (
            ...
            recentLogs.map((log, i) => (
              <div key={i} className="flex items-start gap-4 p-3 border-b border-dim/30 hover:bg-dim/10 transition-colors font-mono text-xs">
                ...
                <div className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold w-24 text-center shrink-0 ${log.type === 'CREATED' ? 'bg-cyan-neon/10 text-cyan-neon' :
